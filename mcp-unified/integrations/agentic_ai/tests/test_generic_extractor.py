#!/usr/bin/env python3
"""
Test Script untuk GenericExtractor
"""
...
async def test_generic():
    """Test scraping dari website umum"""
    
    # URL Wikipedia Indonesia - umumnya mudah di-scrape
    GENERIC_URL = "https://id.wikipedia.org/wiki/Indonesia"
    DOMAIN = "wikipedia"
    TAGS = ["wikipedia", "indonesia", "test"]
    
    print("🚀 GenericExtractor Web Scraping Test")
    print("=" * 60)
    print()
    print(f"📋 Konfigurasi:")
    ...
    try:
        ...
        if result['success']:
            ...
            # Save to file
            filename = f"generic_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            try:
